...
import { saveData } from './bcs/server-save.js'; // added by BCS June 2020
/**
 * Experiment Handler
 *
 * @author Alain Pitiot
	...
	async save({
		...
		if (this._psychoJS.config.experiment.saveFormat === ExperimentHandler.SaveFormat.CSV) {
			...
			if (this._psychoJS.getEnvironment() === ExperimentHandler.Environment.SERVER && this._psychoJS.config.experiment.status === 'RUNNING')
				...
				//offerDataForDownload(key, csv, 'text/csv'); // Commented out by BCS June 2020
				saveData(key, csv, 'text/csv'); // Added by BCS June 2020
		}

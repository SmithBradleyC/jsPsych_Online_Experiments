...
psychoJS.start({
  expName: expName,
  expInfo: expInfo,
  resources:[{name: "Example_Experiment_words.csv", path: "./resources/Example_Experiment_words.csv"}]
  });
...
function trialsStudy1LoopBegin(thisScheduler) {
  ...
  for (const thisTrialsStudy1 of trialsStudy1) {
...
function trialsTestLoopBegin(thisScheduler) {
  ...
  // Schedule all the trials in the trialList:
  for (const thisTrialsTest of trialsTest) {
...
function StudyTrialRoutineBegin(trials) {
  return function () {
    ...
    for (const thisComponent of StudyTrialComponents)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
